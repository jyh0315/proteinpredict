...
function draw() {
  ...
  const dtFactor = dtSeconds * 60; // 60fps 기준 정규화
...
function computeGibbsFreeEnergy() {
  ...
  for (const chain of chains) {
    ...
    for (let i = 0; i < arr.length - 1; i++) {
      ...
      const dist = Math.hypot(b.x - a.x, b.y - a.y);
      const stretch = dist - CONFIG.springRestLength;
      eSpring += 0.5 * CONFIG.springK * stretch * stretch;
    }
  }
  ...
  for (const bond of disulfideBonds) {
    ...
    const stretch = dist - CONFIG.disulfideRestLength;
    eDisulfide += 0.5 * CONFIG.disulfideStrength * stretch * stretch - 8; // 형성 시 큰 음의 기여
  }
  ...
  for (let i = 0; i < charged.length; i++) {
    for (let j = i + 1; j < charged.length; j++) {
      ...
      if (dist < CONFIG.electrostaticRange && dist > 0.001) {
        eElectrostatic += (qa * qb) * CONFIG.electrostaticStrength * CONFIG.electrostaticEnvFactor[environment] * 6 / dist;
      }
    }
  }
